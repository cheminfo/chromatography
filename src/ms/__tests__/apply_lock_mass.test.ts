...
test('simple case', () => {
  ...
  const newLength = chromatogram.getTimes().length / 2;
  ...
  expect(chromatogram.getSeries2D('ms').getData()).toHaveLength(newLength);

  expect(chromatogram.getTimes()).toStrictEqual([1]);
  expect(chromatogram.getSeries2D('ms').getData()[0][1]).toStrictEqual([
    10, 20, 30,
  ]);

  const expectedMass = [100.005, 200.005, 300.005];
  expect(chromatogram.getSeries2D('ms').getData()[0][0]).toBeDeepCloseTo(
    expectedMass,
    3,
  );
});
...
test('different references', () => {
  const chromatogram = new Chromatogram([1, 2, 3, 4, 5, 6], {
    ms: [
      [
  ...
  expect(chromatogram.getSeries2D('ms').getData()[1][1]).toStrictEqual([
    10, 20, 30,
  ]);
  ...
  expect(chromatogram.getSeries2D('ms').getData()[1][0]).toBeDeepCloseTo(
    expectedMass,
    3,
  );
});
...
test('check exceptions', () => {
  const chromatogram = new Chromatogram([1]);
