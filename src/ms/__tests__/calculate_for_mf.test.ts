...
describe('calculateForMF', () => {
  it('basic', async () => {
    ...
    expect(simple.getSeries('C8H3(H+)±0.5').getData()).toStrictEqual([10, 11]);
  });

  it('basic range', async () => {
    await simple.calculateForMF('(CH)0-5C5', { ionizations: 'H+' });
    expect(simple.getSeries('(CH)0-5C5(H+)±0.5').getData()).toStrictEqual([
      10, 11,
    ]);
  });
  ...
  it('check threshold', async () => {
    ...
    expect(simple.getSeries('C8H3(H+)±0.5(0.9)').getData()).toStrictEqual([
      10, 0,
    ]);
  });
  ...
  it('check many ionizations', async () => {
    ...
    expect(simple.getSeries('C8H4(+,H+)±0.005').getData()).toStrictEqual([
      0, 0,
    ]);
  });
  ...
  it('check many ionizations, default slotWidth', async () => {
    ...
    expect(simple.getSeries('C8H4(+,H+)±0.5').getData()).toStrictEqual([
      10, 11,
    ]);
  });
